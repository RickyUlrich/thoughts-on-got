...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    # random tools stuff
    apt-get install -y netcat git cmake exuberant-ctags tree
    apt-get install -y build-essential tmux vim valgrind bc strace ltrace
    ...
    apt-get install -y libncurses5 libstdc++6
